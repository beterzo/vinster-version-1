...
const WensberoepenStep2 = () => {
  ...
  const isStep2Complete = () => {
    ...
    const allQuestionsFilled = [responses.wensberoep_2_werkweek_activiteiten, responses.wensberoep_2_werklocatie_omgeving, responses.wensberoep_2_samenwerking_contacten, responses.wensberoep_2_fluitend_thuiskomen_dag, responses.wensberoep_2_werk_doel, responses.wensberoep_2_leukste_onderdelen, responses.wensberoep_2_belangrijke_aspecten, responses.wensberoep_2_kennis_focus].every(field => field?.trim() !== '' && field !== null);
      ...
      <div className="bg-white shadow-sm">
        <div className="max-w-[1440px] mx-auto px-6 py-4">
          <div className="flex items-center">
            <img alt="Vinster Logo" onClick={() => navigate('/home')} src="/lovable-uploads/4e0bbda6-9e6f-4c55-8880-b6656d264794.png" className="h-20 w-auto cursor-pointer hover:opacity-80 transition-opacity duration-200" />
          </div>
        </div>
      </div>
        ...
        <Card className="rounded-3xl shadow-xl">
          <CardContent className="p-12">
            ...
            <div className="mb-8">
              ...
              <Input id="jobTitle" placeholder="Vul hier de naam van je wensberoep in..." value={jobTitle} onChange={e => handleJobTitleChange(e.target.value)} onBlur={handleJobTitleBlur} className="text-lg border-gray-300 focus:border-blue-900 focus:ring-blue-900" />
            </div>
            ...
            <div className="space-y-6">
              {questions.map((question, index) => <div key={index}>
                  ...
                  <Textarea id={`question${index + 1}`} placeholder="Beschrijf hier je antwoord..." value={answers[`question${index + 1}` as keyof typeof answers]} onChange={e => handleInputChange(`question${index + 1}`, e.target.value)} onBlur={e => handleInputBlur(`question${index + 1}`, e.target.value)} className="min-h-[80px] border-gray-300 focus:border-blue-900 focus:ring-blue-900" />
                </div>)}
            </div>

            <div className="flex justify-between pt-12">
              <Button onClick={handlePrevious} variant="outline" className="border-blue-900 text-blue-900 hover:bg-blue-50">
                Vorige wensberoep
              </Button>
              <Button onClick={handleNext} className={`font-semibold px-8 ${step2Complete ? "bg-yellow-400 hover:bg-yellow-500 text-blue-900" : "bg-gray-300 text-gray-500 cursor-not-allowed"}`} disabled={!step2Complete}>
                Volgende wensberoep
              </Button>
            </div>
          </CardContent>
