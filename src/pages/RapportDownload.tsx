...
const RapportDownload = () => {
  ...
  return <div className="min-h-screen bg-gray-50 font-sans">
      <div className="max-w-4xl mx-auto px-6 py-16">
          ...
          backgroundColor: '#A9C5E2'
            ...
            <div className="grid md:grid-cols-2 gap-8 items-center">
              <div className="text-white">
                ...
                {userReport?.report_status === 'completed' ? <Button onClick={handleDownload} className="bg-white text-blue-900 hover:bg-gray-100 font-bold px-8 py-4 rounded-full text-lg shadow-lg hover:shadow-xl transition-all duration-200" size="lg">
                    <Download className="w-6 h-6 mr-3" />
                    Download je loopbaanrapport
                  </Button> : userReport?.report_status === 'generating' ? <div className="flex items-center space-x-3">
              ...
              <div className="flex justify-center">
                <div className="w-40 h-56 bg-white rounded-xl shadow-2xl transform rotate-3 relative overflow-hidden">
                  ...
                  {userReport?.report_status !== 'completed' && <div className="absolute inset-0 bg-gray-200 bg-opacity-50 flex items-center justify-center">
                      {userReport?.report_status === 'generating' ? <Clock className="w-8 h-8 text-gray-500" /> : <AlertTriangle className="w-8 h-8 text-red-500" />}
                    </div>}
                </div>
              </div>
            </div>
        ...
        {userReport?.report_status === 'completed' && <div className="flex justify-center mb-12">
            <Button onClick={() => navigate("/onderzoeksplan")} className="text-white rounded-xl text-xl py-6 h-auto px-8" style={{
          backgroundColor: '#21324E'
        }} onMouseEnter={e => e.currentTarget.style.backgroundColor = '#2a3b5c'} onMouseLeave={e => e.currentTarget.style.backgroundColor = '#21324E'} size="lg">
        ...
        {/* Simple Status Info */}
        {userReport && <Card className="p-6 bg-white rounded-3xl shadow-sm border border-gray-100 mb-12">
