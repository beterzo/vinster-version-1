      ...
      <div className="max-w-4xl mx-auto px-6 py-12">
        ...
        <div className="bg-white rounded-xl shadow-lg p-8">
          <div className="space-y-4">
            {faqItems.map((item, index) => <Collapsible key={index}>
                <CollapsibleTrigger onClick={() => toggleItem(index)} className="flex items-center justify-between w-full p-4 text-left border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors duration-200">
                  <h3 className="text-lg font-semibold" style={{
                color: '#232D4B'
              }}>
                    {item.question}
                  </h3>
                  <ChevronDown className={`h-5 w-5 transition-transform duration-200 ${openItems.includes(index) ? 'rotate-180' : ''}`} style={{
                color: '#232D4B'
              }} />
                </CollapsibleTrigger>
